<template>
  <div class="aside h-full w-[15%] box-border">
    <div class="avator-box flex ">
      <div class="h-[70px] w-full flex items-center cursor-pointer justify-center" @click="handleLogin" v-if="!userStore.isLogin">
        <el-avatar :size="30" />
        <span class="ml-2 text-[14px]">未登录</span>
      </div>
      <div class="h-[70px] w-full flex items-center cursor-pointer justify-center"  v-else>
      ...
      <template v-for="(menuItem, index) in asideMenuConfig" :key="index">
        ...
        <template v-else>
          <div v-for="item in menuItem.list" :style="{ fontSize: item.asideFontSize + 'px' || '' }"
            :class="['play-list-item']" @click="router.push(item.path)" >
            <i v-if="item.icon" :class="['iconfont', item.icon || '']"></i>
            <span>{{ item.name }}</span>
          </div>
        </template>
        <div v-if="index < asideMenuConfig.length - 1" class="h-[1px] bg-black mx-[10px] mt-[15px]"></div>
      </template>
    </div>
