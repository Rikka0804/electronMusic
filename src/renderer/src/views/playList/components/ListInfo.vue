  ...
  <div class="listInfo flex" v-if="musicStore.currentItem?.coverImgUrl" >
    ...
    <div class="right h-[180px] flex flex-col">
      ...
      <div class="listInfo flex items-center">
        <div class="createPerson flex cursor-pointer items-center mr-[15px]" @click="wentUserInfo(musicStore.currentItem.userId)">
          <div class="avator bgSetting rounded-full size-[25px] mr-[10px]" :style="{ backgroundImage: `url(${musicStore.currentItem.creator.avatarUrl})` }" >
          </div>
      ...
      <div class="listButton mt-auto">
        ...
        <el-button type="info" title="收藏" style="width: 100px;" v-if="userStore.userInfo?.profile.userId !== musicStore.currentItem.userId && !musicStore.currentItem.subscribed">
          <el-icon size="18px" class="mr-[5px]" ><Star /></el-icon>
          {{ formatNumberToMillion(musicStore.currentItem.subscribedCount) }}
        </el-button>
        <el-button type="info" title="取消收藏" style="width: 100px;" v-if="userStore.userInfo?.profile.userId !== musicStore.currentItem.userId && musicStore.currentItem.subscribed">
          <el-icon size="18px" class="mr-[5px]" style="color: yellow;"><StarFilled /></el-icon>
          {{ formatNumberToMillion(musicStore.currentItem.subscribedCount) }}
        </el-button>
        <el-button type="info" title="下载" style="width: 100px;">
          <el-icon size="18px" class="mr-[5px]" ><Download /></el-icon>
          下载
        </el-button>
      </div>
    </div>
